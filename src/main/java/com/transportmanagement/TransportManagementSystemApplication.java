...
public class TransportManagementSystemApplication implements CommandLineRunner {
	...
	private final Logger LOG = LoggerFactory.getLogger(TransportManagementSystemApplication.class);
	...
	public void run(String... args) throws Exception {
		...
		User emloyee = this.userService.getUserByEmailIdAndRoleAndStatus("thuc.admin@demo.com",
				UserRole.ROLE_ADMIN.value(), ActiveStatus.ACTIVE.value());
		...
		if (emloyee == null) {
			...
			User user = new User();
			user.setEmailId("thuc.admin@demo.com");
			user.setPassword(passwordEncoder.encode("123456"));
			user.setRole(UserRole.ROLE_ADMIN.value());
			user.setStatus(ActiveStatus.ACTIVE.value());
